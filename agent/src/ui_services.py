...
def _normalize_timestamp(raw: str) -> Optional[str]:
    """Return a chart-friendly timestamp string.

    Daily bars use ``YYYY-MM-DD``; intraday bars (where the source string
    contains a time component) preserve ``YYYY-MM-DD HH:MM:SS`` so that
    same-day bars are not collapsed on the chart.
    """
    value = str(raw).strip()
    if not value:
        return None
    ...
    if len(value) == 10 and value[4] == "-" and value[7] == "-":
        return value
    # 8-digit compact date
    if len(value) == 8 and value.isdigit():
        return f"{value[:4]}-{value[4:6]}-{value[6:8]}"
    # Datetime string — keep time component for intraday resolution
    if len(value) > 10 and value[4] == "-" and value[7] == "-":
        return value
    return value
...
def _normalize_price_rows(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in rows:
        timestamp = _normalize_timestamp(row.get("timestamp") or row.get("time") or "")
        if not timestamp:
            continue
        normalized.append(
            {
...
def _flatten_data_map(data_map: Dict[str, Any], start_date: str) -> List[Dict[str, Any]]:
    ...
    for code, frame in data_map.items():
        ...
        for timestamp, row in current.iterrows():
            ...
            fmt = "%Y-%m-%d %H:%M:%S" if (ts.hour or ts.minute or ts.second) else "%Y-%m-%d"
