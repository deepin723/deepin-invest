...
_DEFAULT_EXAMPLES: Final[tuple[str, ...]] = (
    "对比 AAPL 和 MSFT 的基本面",
    "回测 BTC 动量策略，2020 年至今",
    "TSLA 2026 年 1 月认购期权的隐含波动率是多少？",
)
...
def print_banner(
    ...
    *,
    ...
    if sessions > 0:
        rows.append(("会话", f"{sessions} 个历史会话（用 /history 浏览）"))
    else:
        rows.append(("会话", "暂无历史会话"))
    ...
    # ── Examples ────────────────────────────────────────────────────────
    console.print(Text(pad) + Text("试试这些：", style=Theme.label))
    ...
    footer.append(" 查看命令 · ", style=Theme.muted)
    footer.append("Ctrl+C", style=Theme.info)
    footer.append(" 清除输入 · ", style=Theme.muted)
    footer.append("Ctrl+D", style=Theme.info)
    footer.append(" 退出", style=Theme.muted)
