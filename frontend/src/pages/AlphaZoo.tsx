...
const ZOO_CARDS: ZooCard[] = [
  {
    ...
    title: "Qlib 158",
    description:
      "微软 Qlib 的 158 个特征库，覆盖动量、波动率、成交量和滚动统计信号。",
    approxCount: 154,
  ...
  },
  {
    ...
    description:
      "Kakushadze (2015) 的 101 个公式化 Alpha，短期横截面信号。",
    approxCount: 101,
  ...
  },
  {
    ...
    title: "GTJA 191",
    description:
      "国泰君安证券 191 个 Alpha 因子，针对 A 股市场调优的技术与微观结构信号。",
    approxCount: 191,
...
const UNIVERSE_OPTIONS = [
  ...
  { value: "btc-usdt", label: "BTC-USDT（加密）" },
];
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground max-w-2xl">
          ...
          点击任意 Alpha 查看公式和源码，或运行横评在指定 Universe 和时间段上评分。
        </p>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-end gap-3 border rounded-xl p-4 bg-card">
        ...
        >
          <Play className="h-3.5 w-3.5" aria-hidden="true" /> 运行横评
        </Link>
      </div>
...
function DetailView({ alphaId }: DetailProps) {
    ...
    return (
      <div className="p-8 max-w-3xl mx-auto space-y-4">
        <Link to="/alpha-zoo" className="text-sm text-muted-foreground hover:text-foreground inline-flex items-center gap-1">
          <ArrowLeft className="h-3.5 w-3.5" aria-hidden="true" /> 返回 Alpha 因子库
        </Link>
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
        ...
        <div className="border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <tbody>
              ...
              <MetaRow label="标的范围" value={metaString(meta, "universe")} />
              <MetaRow label="频率" value={metaString(meta, "frequency")} />
              <MetaRow label="衰减期" value={metaString(meta, "decay_horizon")} />
              <MetaRow label="最小预热 bar 数" value={metaString(meta, "min_warmup_bars")} />
              <MetaRow label="需要板块数据" value={metaString(meta, "requires_sector")} />
      ...
      {/* Source code */}
      <section className="space-y-2">
        <h2 className="text-sm font-medium text-muted-foreground">源代码</h2>
...
function BenchView() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6">
      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-xs text-muted-foreground uppercase tracking-wide">
          <Play className="h-3.5 w-3.5" aria-hidden="true" /> 横评执行器
        </div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">
          在指定标的范围上评分
        </h1>
        <p className="text-sm text-muted-foreground max-w-2xl">
          对选定因子库中的每个 Alpha 在指定标的范围和时间段内计算 IC / IR，并分类为有效 / 反转 / 失效。
        </p>
      </div>
      ...
      >
        ...
        <div>
          <label htmlFor="bench-universe" className="text-xs text-muted-foreground block mb-1">标的范围</label>
          <select
            id="bench-universe"
            value={universe}
        ...
        <div>
          <label htmlFor="bench-period" className="text-xs text-muted-foreground block mb-1">时间段</label>
          <input
            id="bench-period"
            value={period}
        ...
        <div>
          <label htmlFor="bench-top" className="text-xs text-muted-foreground block mb-1">Top N</label>
          <input
            id="bench-top"
            type="number"
            min={1}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <TopTable title="IR 最高前 5" rows={result.top5_by_ir || []} />
        <TopTable title="反转最多" rows={(result.dead_examples || []).slice(0, 3)} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b">
              ...
              <th className="text-right px-4 py-2 text-xs text-muted-foreground font-medium">均值 IC</th>
